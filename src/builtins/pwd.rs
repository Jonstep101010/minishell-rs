...
pub unsafe extern "C" fn builtin_pwd(
	...
	let mut tmp: *mut libc::c_char = getcwd(std::ptr::null_mut::<libc::c_char>(), 0);
	let mut env_pwd: *mut libc::c_char =
		get_env((*shell).env, b"PWD\0" as *const u8 as *const libc::c_char);
	if env_pwd.is_null() {
