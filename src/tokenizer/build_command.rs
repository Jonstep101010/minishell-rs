...
use crate::{
	parser::split_outside_quotes::{split_non_quoted, split_outside_quotes},
	t_shell, t_token,
};
use ::libc;
use libc::free;
use libft_rs::ft_strdup::ft_strdup;
use libutils_rs::src::array::{
	append_str::append_str_arr_free, arr_free::arr_free, arr_len::arr_len,
};
...
pub unsafe fn get_cmd_arr_token(mut token: *mut t_token) -> *mut *mut libc::c_char {
	...
	while !((*((*token).cmd_args).add(i)).elem).is_null() {
		if (*((*token).cmd_args).add(i)).type_0 != crate::e_arg::REDIR {
